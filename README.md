...
1. Set `**OPENROUTER_API_KEY**` (see `.env.example` for optional variables such as Tavily or `BOARDROOM_CREDENTIAL_KEY`).
  - Keep a `**.env` in this repo** (project root) and run CLI commands from there, or export variables in your shell.

On startup, the CLI loads `**.env`** from the **current working directory** via `python-dotenv` (standard behavior).

1. Optionally store an encrypted key for when the env var is unset:
...
1. Edit `**config.yaml`** for models, rate limiting, `vector_store`, and `web_search`.
...
Run the FastAPI + React UI in a container (container port `8000`, host port defaults to `18000` to avoid common Windows port reservations).

1. Copy `.env.example` to `.env` and set `OPENROUTER_API_KEY` (Compose reads `.env` for variable substitution).
...
3. Open [http://localhost:18000](http://localhost:18000).

Set `BOARDROOM_HOST_PORT` in `.env` if you want a different host port.

Data is persisted in named volumes: `.boardroom` (vector store, optional encrypted keys, custom agents) and `transcripts` (meeting outputs), under `/app` per `config.yaml`.

The image runs `uvicorn` on `0.0.0.0:8000` (required for host access). Do not expose the container to untrusted networks without a reverse proxy and authentication.
